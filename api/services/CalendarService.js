...
class CalendarService {
    ...
    async inviteUserByCalendarIdAndEmail(inviter_id, inviter_name, calendar_id, email) {
        ...
        const token = this.tokenService.generateRefreshToken({ calendar_id: calendar_id, user_id: user._id });
        this.mailService.sendInvitation(email, process.env.SERVER_URL + '/api/calendars/go-into/' + token.refreshToken, inviter_name, calendar.name);
    }
    ...
    async removeUserFromCalendar(remover_id, user_id, calendar_id) {
        ...
        const events = await EventModel.find({ $and: [{ owner_id: user_id }, { calendar_id: calendar_id }] });
    ...
    // return calendar id
    async addUserToCalendarByToken(token) {
        console.log(`Add user to the calendar`);
        if (!token) {
            throw ErrorHandler.BadRequest('Invalid token');
        }

        token = this.tokenService.validateRefreshToken(token);
        if (!token) {
            throw ErrorHandler.BadRequest('Token is not valid');
        }

        const calendar_id = token.calendar_id;
